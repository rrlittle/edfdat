'''
	...
	1. call parse_edf
		...
		notable terminology: 
			- trial: stolen from SR research. the experiment is broken up into trials
						usually there is one recording block which repeats some number 
						of times. each time is a trial
			- trial_set : all the lines from the file that correspond to one trial
...
def msg_parser(trial_str, *args, **kwargs):
    ...
    try:
        ...
        edflog.error('Attempting to parse message string ' + 
...
def sample_parser(trial_str, 
	...
	**kwargs):
	...
	split_tr = re.split(r'\s+', trial_str)
	edflog.debug('In parse sample. split string = %s'%split_tr)
	...
	try:
		...
		edflog.error('Trial str (%s) did not meet expectations based on '%trial_str
...
def parse_trial(trial_str, **kwargs):
	''' This takes a trial string which is a single line from file
	...
	def err(): # we call the same error message a few times here. 
		edflog.debug('in parse_trial:{%s} is neither a sample nor a MSG.'%trial_str)
		return None
	...
	# figure out what kind of line this is. a couple different type possible
	# sample, msg, EFIX, etc...
	stype = None
	try:
	...
	if stype in typeparsers: 
		try:
			return typeparsers[stype](trial_str, **kwargs) ########## NEED TO UPDATE monocle should reflect whether theres a monocle or not as set by global variable or something...
...
def parse_trial_set(tr_list, 
	...
	**kwargs):
	...
	for trial in tr_list:
		parsed_trial = parse_trial(trial, **kwargs)
...
def get_trial_sets(fp,reset=True):
	...
	for line in fp: # go line by line 
		if emptystr(line): continue
		if matches_trial_start(line):
		   trials.append([])
		trials[-1].append(line.strip()) # add line to 
	fp.seek(0)
	return trials
...
def parse_edf(edfpath=None):
	...
	edflog.warn('opening file %s'%edfpath)
	f = open_edf(edfpath)
	...
	for i,tr_set in enumerate(sets): # parse each trial
		edflog.warn('processing trial %s of %s'%(i+1,len(sets)))
		...
		edflog.info('\tfound %s samples'%len(samples))
		edflog.info('\tfound %s messages '%len(messages))
		edflog.info('\tcould not parse %s samples'%len(dropped)) 
		tr_dict = {
			'samples':samples,
			'dropped': dropped,
			'messages': messages
...
class unitests():
	...
	def openedfprint(self):
		...
		parsed = parse_edf('E:/Users/russell/Documents/work/populin/edfdat_learning_environment/feedback.txt')
